  ...
  return (
    <>
      <section className="relative h-[70vh] overflow-hidden">
        ...
        />
        <div className="absolute inset-0 bg-linear-to-t from-[#0F0F0F]/80 via-[#0F0F0F]/20 to-transparent" />
      ...
      <section className="bg-background px-6 py-16 lg:px-8 lg:py-24">
        <div className="mx-auto max-w-7xl">
          <div className="grid gap-12 lg:grid-cols-3">
            ...
            <FadeIn delay={0.2} className={project.description ? "" : "lg:col-start-3"}>
                ...
                <ul className="mt-6 flex flex-col gap-4">
                  ...
                  {project.camera && (
                    <li className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-xs text-muted-foreground">Boîtier</p>
      ...
      <section className="border-t border-border bg-card px-6 py-12 lg:px-8">
        <div className="mx-auto flex max-w-7xl items-center justify-between">
          {prevProject ? (
            <Link href={`/portfolio/${prevProject.id}`} className="group flex items-center gap-3">
              <ArrowLeft className="h-4 w-4 text-muted-foreground transition-transform group-hover:-translate-x-1" />
              <div>
                <p className="text-xs text-muted-foreground">Projet précédent</p>
                <p className="font-serif text-lg font-medium text-foreground">{prevProject.title}</p>
              </div>
            </Link>
          ) : <div />}

          {nextProject ? (
            <Link href={`/portfolio/${nextProject.id}`} className="group flex items-center gap-3 text-right">
              <div>
                <p className="text-xs text-muted-foreground">Projet suivant</p>
                <p className="font-serif text-lg font-medium text-foreground">{nextProject.title}</p>
              </div>
